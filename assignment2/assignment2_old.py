...
class Rabbit(Agent):
    ...
    def eat(self, grass):
        if grass is not None and self.pos.distance_to(grass[0].pos) < self.config.eat_range:
            ...
            if "pregnant" not in self.states and random.random() < self.config.rabbit_reproduction_rate:
    ...
    # This is 2 rabbit having offspring function (with no sexuality)
    ...
    def hungry(self):
        ...
        if self.hungry_counter > self.config.rabbit_starving_thershold:
            self.kill()
            self.config.rabbit_num -= 1
        elif self.hungry_counter > self.config.rabbit_starving_thershold * self.config.hungry_thershold:
...
class Fox(Agent):
    ...
    def hungry(self):
        if self.hungry_counter > self.config.fox_starving_thershold * self.config.hungry_thershold and "fine" in self.states:
            self.states = ["hungry" if i == "fine" else i for i in self.states]
